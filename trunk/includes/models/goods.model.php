...
class GoodsModel extends BaseModel
{
    ...
    function get_list($params = array(), $scate_ids = array(), $desc = false, $no_picture = true)
    {
		...
		//Add cate_id_1 by MingFONG for clothing style at 2014-09-22
        $fields = "g.goods_id, g.store_id, g.type, g.goods_name, g.cate_id, g.cate_name, g.brand, g.spec_qty, g.spec_name_1, g.spec_name_2, g.if_show, g.closed, g.add_time, g.recommended, g.default_image, g.cate_id_1, " .
		...
		foreach ($goods_list as $key => $goods) {
			if($promotion_mod->goods_has_promotion($goods['goods_id'])){
				$goods_list[$key]['pprice'] = $promotion_mod->get_promotion_price($goods['goods_id'], $goods['spec_id']);
				$goods_list[$key]['discount'] = round($goods_list[$key]['pprice']/$goods['price'],2)*10;
			}
		}
		
		/* Get goods spec by MingFONG */
		...
		foreach ($goods_list as $key => $goods) {
			if ($goods)
			{
				...
				$goods_list[$key]['_specs'][] = $specs[$goods['default_spec']];
				unset($specs[$goods['default_spec']]);
				$goods_list[$key]['_specs'] = array_merge($goods_list[$key]['_specs'], array_values($specs));
				/* 商品图片 */
    ...
    function get_info($id)
    {
        $goods = $this->get(array(
            'conditions' => "goods_id = '$id'",
            'join'       => 'belongs_to_store,has_tips',
            'fields'     => 'this.*, store.state,tips_content'
        ));
        if ($goods)
        {
            /* 商品规格 */
