...
class ECBaseApp extends BaseApp
{
	...
	function Sms_Get($method='', $phone='',$text='',$user_id=0)
	{
		...
		if($method == 'SMS_Num') {
			...
			} else $gateway = 'http://sms.webchinese.cn/web_api/SMS/?Action=SMS_Num&';
		...
		else // 发送短信
		{
			...
			if($res>0)
			{
				...
				$mod_msgstatistics->edit(0,"used = used +".$res); //已经使用短信数，写入统计
				if($user_id>0)
				{
					$mod_msg = &m('msg');
					$mod_msg->edit("user_id='{$user_id}'","num = num - ".$res);//如果是用户发送减少用户的短信数
				}
				$add_msglog = array(
					'user_id' => $user_id,
					'to_mobile' => $phone,
...
class BaseVisitor extends Object
{
    ...
    {
        if (!empty($_SESSION[$this->_info_key]['user_id']) || !empty($_SESSION[$this->_info_key]['waiter_id']))
        {
    ...
    //店小二前台页面登录状态失效 by andcpp
    function waiter_logout()
    {
    	$this->has_login    = false;
        //unset($_SESSION[$this->_info_key]['manage_store']);
    }
