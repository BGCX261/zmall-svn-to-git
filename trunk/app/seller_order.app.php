        ...
        else
        {
			...
			if (file_exists($filename))
			{
				if (!$is_edit)
				{
					...
					$row_order_extm = $mod_order_extm->get("order_id={$order_id}");
					$mobile = $row_order_extm['phone_mob']; //在订单中取收货人留手机号
					$user_id = $order_info['seller_id'];
					$row_msg = $mod_msg->get("user_id='{$user_id}'");
					$smsText = sprintf(Lang::get('sms_send'),$order_info['order_sn'],$order_info['seller_name']);	
					...
					if($row_msg['state'] == 1 && $checked_functions['send'] == 1 && $row_msg['num']>0 && !empty($mobile) && !empty($smsText))
					{
