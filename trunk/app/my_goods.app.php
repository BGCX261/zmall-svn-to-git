...
var $_region_mod;
    ...
    function add()
    {
        ...
        if (!IS_POST)
        {
			 ...
			 $store_tips = &m('storetips');
			 $tips = array();
			 $tips = $store_tips->find();
             $this->assign('tips_option',$tips);
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
			...
			$goods['dest_ids'] = $goods['goods_dests'];
			$dest_ids = explode('|',$goods['goods_dests']);
			$goods['dests'] = '';
			foreach($dest_ids as $dest)
			{
				$goods['dests'] .= $this->_region_mod->get_region_name($dest) .' ';
			}
