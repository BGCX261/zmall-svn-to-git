...
class Seller_promotionApp extends StoreadminbaseApp
{
    ...
    function index()
    {
		if(!empty($_GET['pro_name'])){
			$conditions = " AND (pro.pro_name LIKE '%".trim($_GET['pro_name'])."%' OR g.goods_name LIKE '%".trim($_GET['pro_name'])."%') ";
		}
		else{
			$conditions = '';
		}
        ...
        $promotion_list = $this->_promotion_mod->find(
            array(
				...
				'fields' => 'pro.*,g.default_image,g.price,g.default_spec,g.goods_name',
                'count' => true
            )
        );
