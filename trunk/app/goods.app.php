...
class GoodsApp extends StorebaseApp
{
    ...
    function index()
    {
		...
		if(!empty($data['goods']['goods_dests']))
		{
			$ids = explode('|',$data['goods']['goods_dests']);
			$area = $this->_get_deliery_region($ids,$area); //配送范围过滤
		}
    	...
    	function _get_deliery_region($ids,$area)
	{
		foreach($area as $key=>$val)
		{
			if(!in_array($key,$ids)) //省份id在ids内跳过
			{
				foreach($val['cities'] as $k=>$city)
				{
					if(!in_array($k,$ids)) //过滤城市id不在ids内
					{
						unset($area[$key]['cities'][$k]);
					}
				}
				if(count($area[$key]['cities']) == 0)  //过滤没有城市的省份
				{
					unset($area[$key]);
				}
			}
    ...
    function sizechart()
    {
        ...
        $this->display('goods.sizechart.html');
    }
	/* 销售记录 */
    function saleslog()
    {
