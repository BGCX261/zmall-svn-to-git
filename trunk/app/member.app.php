...
class MemberApp extends MemberbaseApp
{
    ...
    function register()
    {
        ...
        else
        {
			...
			{
				$this->show_warning('err_check_code_err');
                return;
			}
			...
			$phone_mob=$_POST['phone_mob'];//by cengnlaeng
			...
			$waiter_info = $model_waiter->get("waiter_name='{$user_name}'");
			if($waiter_info)
			{
            ...
            $ms =& ms(); //连接用户中心
            $user_id = $ms->user->register($user_name, $password, $email,array('phone_mob'=>$phone_mob,'r_id'=>$_POST['region_id'],'r_name'=>$_POST['region_name']));//by cengnlaeng 
    ...
    function phone(){
        ...
        else
        {
			...
			$ms =& ms();
            $res = $ms->user->auth($this->visitor->get('user_name'), $orig_password);
            if (!$res)
            {
                /* 未通过验证，提示错误信息 */
                $this->show_warning($ms->user->get_error());
			...
			$model_member = &m('member');
			$result = $model_member->edit($user_id,array('phone_mob' => $phone));
            if (!$result)
            {
                $this->show_warning($model_member->get_error());
